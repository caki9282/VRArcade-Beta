...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRLever : XRBaseInteractable
    {
        ...
        [Tooltip("The train object to be moved")]
        public Transform train;

        [Tooltip("Waypoints for the train to follow")]
        public Transform[] waypoints;

        [Tooltip("Maximum speed of the train")]
        public float maxTrainSpeed = 5f;

        private float currentTrainPosition = 0f;
        ...
        float GetLeverAngle()
        {
            Vector3 handleEuler = handle.localRotation.eulerAngles;
            return handleEuler.x > 180 ? handleEuler.x - 360 : handleEuler.x;
        }

        void MoveTrain(float position)
        {
            int startPointIndex = Mathf.FloorToInt(position);
            int endPointIndex = Mathf.CeilToInt(position);

            if (startPointIndex >= waypoints.Length - 1)
                startPointIndex = waypoints.Length - 2;

            if (endPointIndex >= waypoints.Length)
                endPointIndex = waypoints.Length - 1;

            Vector3 startPoint = waypoints[startPointIndex].position;
            Vector3 endPoint = waypoints[endPointIndex].position;

            float lerpFactor = position - startPointIndex;

            train.position = Vector3.Lerp(startPoint, endPoint, lerpFactor);
        }
        ...
        void Update()
        {
            // Update train position based on the lever value
            if (train != null && waypoints.Length > 1)
            {
                // Map the lever's angle to a normalized value (0 to 1)
